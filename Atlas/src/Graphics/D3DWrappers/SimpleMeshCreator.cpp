...
namespace Atlas 
{
	...
	IndexedTriangleList Cube::MakeIndipendentFaces()
	{
		constexpr float side = 0.5f;
	...
	IndexedTriangleList Plane::MakeTessalated(uint divisionsX, uint divisionsY)
	{
		AT_CORE_ASSERT(divisionsX != 0, "Cannot make a plane with 0 tessellation in the x direction");
		AT_CORE_ASSERT(divisionsY != 0, "Cannot make a plane with 0 tessellation in the y direction");
	...
	IndexedTriangleList Cone::MakeTessalated(uint baseDivisions)
	{
	...
	IndexedTriangleList Cone::MakeTessalatedIndipendentFaces(uint baseDivisions)
	{
		AT_CORE_ASSERT(baseDivisions >= 3, "Cannot make a cone with less then 3 base vertices");
	...
	IndexedTriangleList Prism::MakeTessalated(uint baseDivisions)
	{
	...
	IndexedTriangleList Sphere::MakeTessalated(uint latitudeDivisions, uint longitudeDivisions)
	{
		AT_CORE_ASSERT(latitudeDivisions >= 3, "Cannot make a sphere with less then 3 latitude tessellation");
		AT_CORE_ASSERT(longitudeDivisions >= 3, "Cannot make a sphere with less then 3 longitude tessellation");
	...
	IndexedTriangleList::IndexedTriangleList(std::vector<DirectX::XMFLOAT3> verteciesIn, std::vector<unsigned short> indeciesIn)
	...
	void IndexedTriangleList::Transform(DirectX::XMMATRIX transform)
	{
		//Change positions
		for (auto& vertex : m_Vertecies)
		{
	...
	std::vector<DirectX::XMFLOAT3> IndexedTriangleList::GetVertecies() { return m_Vertecies; }

	std::vector<NormalVertex> IndexedTriangleList::GetNormalVertecies() { return m_NormalsVerticies; }

	std::vector<unsigned short> IndexedTriangleList::GetIndecies() { return m_Indecies; }
